...
### Requirement: 提醒开启与授权
用户 SHALL 能在设置弹层「主动提醒」中选择任意时间（time picker，分钟级）并开启每日提醒。开启动作的 tap 回调中 SHALL 同步调用 `requestSubscribeMessage`（模板「日报提醒」），这是全产品**唯一**允许出现订阅授权弹窗的位置；界面文案 SHALL 引导用户勾选"总是保持以上选择"。授权返回 `accept` 后 SHALL 调用云函数写入订阅记录并使额度 +1。
