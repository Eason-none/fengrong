...
- [x] 1.3 验证个人主体云调用 `cloud.openapi.subscribeMessage.send` 权限（2026-07-16 真机实测发送成功，权限无碍）
...
- [x] 5.2 小程序后台"用户隐私保护指引"补 openid 收集声明——移交 product_handoff 上线前清单第 9 条跟踪（提审前完成，不阻塞本变更归档）
- [x] 5.3 `manual_acceptance_checklist_v2.md` §6 恢复为三项 + 新增 6.4~6.8 提醒验收项；`product_handoff.md` 上线清单第 9 条加回主动提醒三项（含翻案记录）

## 6. 端到端验证（真机）

- [x] 6.1 开启提醒 → 云数据库出现记录 → 到点收到服务通知（9:16 设定 9:20 送达，5 分钟窗口符合预期；12:00 同日重设不重发也验证通过）→ 点击回到小程序（2026-07-16 用户验收：落回原页面的观感被认可，"在哪开的通知就还在哪"更顺）
- [x] 6.2 触发续票挂点（已勾"总是保持"）→ 无任何弹窗 → 数据库 quota 0→1（2026-07-16 用户验收通过）
- [x] 6.3 额度耗尽后设置显示"已暂停"→ 重新开启静默恢复（勾过"总是保持"故无弹窗，符合设计）；关闭提醒 → 数据库记录消失（2026-07-16 用户验收通过）
